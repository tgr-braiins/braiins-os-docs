 ...
 * *bos_scanner*: контейнер со сканером ssh, который сканирует майнеры с прошивкой Braiins OS+ в определенной сети с целью подготовки данных для информационной панели Grafana **Farm Dashboard**,
...
Braiins рекомендует следить за репозиторием Braiins Farm Proxy Github, чтобы получать уведомления, в случае доступности новой версии. Чтобы перейти на более новую версию, просто запустите команду ``git pull origin master`` в терминале Linux. Если вы внесли какие-либо изменения в файлы конфигурации, вы можете сохранить измененные файлы или стешнуть их с помощью команды ``git stash`` перед обновлением. Если вы внесли какие-либо изменения в файл docker-compose.yml, вам нужно будет внести их снова после обновления, поскольку ``git pull`` извлекает docker-compose.yml из репозитория Github.
...
4. Удалите сохраненные данные мониторинга: ``docker volume prune``,
5. Удалите папку: ``rm -rf farm-proxy/``.

...
======================================
  ...
  * 在**安装**部分，选择刚创建的txt文本文件来填写**矿机名**选项并点击**启动**。
...
设置电源功率限制                       --psu-power-limit [PSU_POWER_LIMIT]   设置（以瓦为单位）的电源功率限制
设置矿池用户                           --pool-user [POOL_USER]               配置SLush Pool矿池的用户名和矿工名"
最新稳定发布版                         *N/A* - 默认选项                       默认选择，推荐的选项
开源版本                               --open-source         		 用于安装开源版本 (只有在 **open-source**和**nightly**版本上)
预先发布版                             --nightly             		用于安装预先发布版 (只有在 **open-source**和**nightly**版本上)
自定义固件下载地址                     --feeds-url [FEEDS_URL]		     自定义固件下载URL链接地址 (只有在 **open-source**和**nightly**版本上)
固件版本                              --fw-version [FW_VERSION]	         选择具体固件版本
不开启自动升级                         --no-auto-upgrade                     不开启自动升级（不推荐）
不保留现有矿池配置                     --no-keep-pools                       不保留（转移）矿机的原矿池配置
保留现有网络配置                       *N/A* - Default Option                保留矿机的原网络配置，默认选择（推荐）
...
*不适用* - 未应用                      -h, --help                            显示帮助信息并退出
*不适用* - 未应用                      --backup                              在安装前备份矿机（NAND内置储存上的原固件和配置）
*不适用* - 未应用                      --no-nand-backup                      不备份矿机NAND内置储存（仍备份配置
*不适用* - 未应用                      --no-wait                             直到系统完全更新或重启完毕不等待
*不适用* - 未应用                      --dry-run                             执行所有的更新步骤但不实际进行更新
====================================  ====================================  ============================================================
...
=====================================
  ...
  * 创建一个txt文本文件，并将文件命名为"listOfMiners"，然后在文件内输入您想执行操作的矿机的IP地址， **一个IP地址一行** ！保存文本文件后，再将文件后缀从".txt"改为".csv"。并确定此文件和BOS工具箱都放在同一路径下（同一文件夹中）。 
  * 一旦下载了BOS+工具箱，双击（Windows）或在命令行使用``./bos-toolbox``命令（Linux）来运行它。
  * 在**升级**部分，选择刚创建的txt文本文件来填写**矿机名**选项并点击**启动**。
...
GUI选项                               参数                                   描述
====================================  ====================================  ============================================================
升级包                 	            PACKAGE_NAME                         固件升级包名（选择升级到哪个固件版本）
密码                                  -p PASSWORD, --password PASSWORD      矿机密码
矿场ID                                --bos-mgmt-id [BOS_MGMT_ID]           设定Braiins OS+管家标识符
...
::
  ...
  bos-toolbox.bat update listOfMiners.csv 固件
...
========================================
  ...
  * 一旦下载了BOS+工具箱，双击（Windows）或在命令行使用``./bos-toolbox``命令（Linux）来运行它。
  * 在**卸载**部分，选择刚创建的txt文本文件来填写**矿机名**选项并点击**启动**。
您可以使用下方的 **参数** 调整卸载进程：
...
====================================  ====================================  ============================================================
密码                                  -p PASSWORD, --password PASSWORD      矿机密码
默认原厂固件                           *不适用* - 默认选项                    默认设置
自定义固件下载地址                      --feeds-url [FEEDS_URL]		    自定义固件下载URL链接地址
*不适用* - 未应用                      --nand-restore			       使用之前的备份对整个NAND进行恢复
*不适用* - 未应用                      BACKUP_PATH                           指定从一个目录或一个tgz文件恢复备份的路径
*不适用* - 未应用                      --h, --help                           显示帮助信息并退出
====================================  ====================================  ============================================================
...
**重点：** 
*BACKUP_PATH*是可选的。仅与*--nand-restore*参数一起使用。**不建议**从备份中恢复固件。
...
===========================================
  ...
  * 创建一个txt文本文件，并将文件命名为"listOfMiners"，然后在文件内输入您想执行操作的矿机的IP地址，一个IP地址一行！（矿机的IP地址在矿机网页端界面中的 *Status（状态）-> Overview（总览）中可以进行查询）。保存文本文件后，再将文件后缀从".txt"改为".csv"。确定此文件和BOS工具箱都放在同一路径下（同一文件夹中）。 
  * 一旦下载了BOS+工具箱，双击（Windows）或在命令行使用``./bos-toolbox``命令（Linux）来运行它。
  * 在**配置**部分，选择刚创建的txt文本文件来填写**IP地址批量列表**选，并点击**启动**
...
详情                                  -v, --verbose                         报告网络错误
*不适用* - 未应用                      -h, --help                            显示帮助信息并退出
*不适用* - 未应用                      -j JOBS, --jobs JOBS                  网络扫描并行工作数
====================================  ====================================  ============================================================
...
================================================
  ...
  * 创建一个txt文本文件，并将文件命名为"listOfMiners"，然后在文件内输入您想执行操作的矿机的IP地址，一个IP地址一行！（矿机的IP地址在矿机网页端界面中的 *Status（状态）-> Overview（总览）中可以进行查询）。保存文本文件后，再将文件后缀从".txt"改为".csv"。确定此文件和BOS工具箱都放在同一路径下（同一文件夹中）。 
  * 一旦下载了BOS+工具箱，双击（Windows）或在命令行使用``./bos-toolbox``命令（Linux）来运行它。
  * 在**监听**部分，点击**启动**来监听矿机识别广播（当按下IP report键时） ::
...
================================================
  ...
  * 创建一个txt文本文件，并将文件命名为"listOfMiners"，然后在文件内输入您想执行操作的矿机的IP地址，一个IP地址一行！（矿机的IP地址在矿机网页端界面中的 *Status（状态）-> Overview（总览）中可以进行查询）。保存文本文件后，再将文件后缀从".txt"改为".csv"。确定此文件和BOS工具箱都放在同一路径下（同一文件夹中）。 
  * 一旦下载了BOS+工具箱，双击（Windows）或在命令行使用``./bos-toolbox``命令（Linux）来运行它。
  * 在**命令**部分，通过选择刚创建的txt文本文件来填写**矿机名**选项，填写要运行的命令并点击**启动**
...
密码                                  -p PASSWORD, --password PASSWORD      矿机密码
*不适用* - 未应用                      -h, --help                            显示帮助信息并退出
*不适用* - 未应用                      -j JOBS, --jobs JOBS                  并行工作数
*不适用* - 未应用                      -a, --auto                            如RPC不可用，则使用SSH
*不适用* - 未应用                      -l, --legacy                          使用SSH
*不适用* - 未应用                      -L, --no-legacy                       使用RPC
====================================  ====================================  ============================================================
...
============================================
  ...
  * 创建一个txt文本文件，并将文件命名为"listOfMiners"，然后在文件内输入您想执行操作的矿机的IP地址， **一个IP地址一行** ！保存文本文件后，再将文件后缀从".txt"改为".csv"。并确定此文件和BOS工具箱都放在同一路径下（同一文件夹中）。 
  *  一旦下载了BOS+工具箱，双击（Windows）或在命令行使用``./bos-toolbox``命令（Linux）来运行它。
  ** 在**解锁**部分，通过选择刚创建的txt文本文件来填写**矿机名**选项并点击 **启动**. ::
...
密码                                   -p PASSWORD, --password PASSWORD      矿机登录密码
*不适用* - 未应用                       --h, --help                           显示帮助信息并退出
*不适用* - 未应用                       --port PORT                           蚂蚁矿机网页后台解锁用端口
*不适用* - 未应用                       --ssl                                 是否使用SSL协议
====================================  ====================================  ============================================================

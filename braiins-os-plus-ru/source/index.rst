...
Это дополнительный релиз, в котором исправлены мелкие ошибки, связанные с подключениями к пулам.

* Все модели

   * [исправление ошибок] Незначительные ошибки, связанные с подключением к пулам
  
22.08
 ---------------------------

 Это основной релиз, который включает в себя BeagleBone Black Control Board для общего пользования и режим Hashrate Target для автонастройки.

 * Семейство Antminer X19

   * [функция] Целевой режим хэшрейта для автонастройки
   * [улучшение] Улучшенные кривых мощности для S19j PRO и S19 PRO

 * Семейство Antminer X17

   * [функция] Целевой режим хэшрейта для автонастройки
   * [улучшение] Скорректированы значения ограничения мощности по умолчанию для семейства Antminer X17.

 **ПРИМЕЧАНИЕ:**

 * Ограничения мощности по умолчанию для семейства X17 теперь согласованы со стандартными ограничениями мощности в зависимости от варианта машины. Предыдущее ограничение мощности по умолчанию в 1700 Вт теперь составляет 2200 Вт, что более точно соответствует потреблению и выходной мощности хешрейта стандартным спецификациям.

 * **ВАЖНО:** Если в настоящее время вы используете много устройств X17 с пределом мощности по умолчанию, это обновление приведет к значительному увеличению энергопотребления. Чтобы предотвратить это, вы можете просто немного изменить ограничение мощности на ваших машинах (например, до 1710 Вт), чтобы обновление не вызвало каких-либо серьезных изменений.

 * Если вы устанавливаете Braiins OS+ на стандартное устройство или используете ограничение мощности, определяемое пользователем, это изменение никак не повлияет на вас.


22.06
---------------------------

Это основной выпуск только для плат управления Beaglebone.

* Семья Antminer

   * [функция] Добавлена поддержка плат управления на основе Beaglebone (включая тюнер, но исключая функцию автоматического обновления)

**Примечание:** этот релиз не будет работать на майнерах, кроме платы управления Beaglebone.

**Примечание:** не знаете, какая у вас плата управления? Проверьте `здесь <https://braiins.com/blog/bitcoin-mining-control-board-variations>`_

22.05
---------------------------

Это второстепенный выпуск, содержащий улучшенные для некоторых моделей Antminers.

* Семья Antminer

   * [функция] Добавлена матрица кривой мощности для моделей S19J и S19 pro, поэтому оценка энергопотребления теперь более точная.

...
::
   ...
   plus_beta.py [-h] [--pool POOL] [--user USER.WORKER]
                [--password PASSWORD] [--power-limit POWER_LIMIT]
                [--image IMAGE] [-f] [--no-wait] [--no-backup]
                {install,upgrade} hostname [hostname ...]

** Позиционные аргументы:**

  * {install,upgrade}     Install mode
      * ``install`` для установки Braiins OS + поверх стоковой прошивки
      * ``upgrade`` для установки Braiins OS + поверх предыдущей версии Braiins OS
  * hostname              Hostname или IP адрес выбранного майнера

** Опциональные аргументы:**

  -h, --help                  Показать это справочное сообщение и выйти
  --pool POOL                 Адрес пула
  --user USER.WORKER          Имя пользователя на пуле
  --password PASSWORD         Пароль для входа в пул или 'ask' для интерактивной подсказки
  --power-limit POWER_LIMIT   Предельная мощность блока питания в ваттах (мин. 100 и макс. 5000)
  --image IMAGE               Postupgrade файл изображения
  -f, --force                 Продолжить с несовместимым обновлением
...
* *name* - Имя группы пула (объяснено в разделе *Pool Groups* ниже)
...
* *url* - Обязательный аргумент для URL сервера в формате <HOSTNAME:PORT>
...
::
   ...
   [[group.pool]]
   enabled = true
   url = 'stratum2+tcp://v2.stratum.slushpool.com:3337'
   user = 'userA.worker'
...
**Target temperature** это температура, которую старается поддерживать майнер (*по умолчанию* **85°C**).
**Hot temperature** порог, при котором вентиляторы начинают работать на 100% (*по умолчанию* **95°C**).
**Dangerous temperature** порог, при котором BOSminer отключается, чтобы предотвратить перегрев и повреждение майнера (*по умолчанию* **105°C**).
...
* *target_temp* - Установка целевой температуры в градусах Цельсия (default=85.0). Эта опция используется ТОЛЬКО когда 'temp_control.mode' установлен на 'auto'!
* *hot_temp* - Установка температуры в градусах Цельсия (default=95.0). Когда майнер достигнет этой температуры, скорость вентилятора установится на 100%.
* *dangerous_temp* - Установка опасных температурных пределов в градусах Цельсия (default=105.0). Когда майнер достигнет этой температуры, майнинг отключится! ** ПРЕДУПРЕЖДЕНИЕ:** слишком высокое значение этого параметра может повредить устройство!
...
Тюнинг может быть настроен либо через веб-интерфейс, либо в файле конфигурации ``/etc/bosminer.toml``.

Чтобы изменить конфигурацию через веб-интерфейс, зайдите в *Miner -> Configuration* меню и редактируйте *Autotuning* секцию.

Чтобы внести изменения в файл конфигурации, подключитесь к майнеру через SSH и отредактируйте файл ``/etc/bosminer.toml``. Синтаксис следующий:

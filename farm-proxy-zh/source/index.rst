...
Braiins矿场代理是一个免费的、独立的工具，在矿场本地运行。代理的目的是节约带宽，同时在每个矿工上聚合SHA256算力，并将其路由到目标目的，就是矿池。矿工被配置为连接到代理。Braiins矿场代理可以被设置为连接到几个目标矿池，并有备份矿池作为故障转移。代理优化矿池的IP地址，并选择具有最低延迟或数据包丢失的端点。如果使用Braiins Pool矿池作为端点，代理还能对矿池的连接进行加密，以提高隐私和安全。
 ...
 * 如果目标终端是Braiins Pool矿池， 那就支持 **加密连接** 来保证数据隐私并防止算力劫持。
